          ...
          <nav className="flex flex-col gap-2 px-4 pt-6">
            {menu.map((item, index) => {
              if ('drawer' in item && item.drawer) {
                ...
                return (
                  <Drawer.NestedRoot key={`drawer-${index}-${item.name}-${item.title ?? ''}`}>
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    item.primary
                      ? 'bg-linear-to-r from-blue-100/80 via-blue-100/40 to-transparent font-normal text-blue-600 dark:from-blue-700/20 dark:via-blue-600/10 dark:to-transparent dark:text-white [&>svg]:stroke-3 [&>svg]:text-blue-600 dark:[&>svg]:stroke-white dark:[&>svg]:stroke-1 dark:[&>svg]:text-white'
