  ...
  return (
    <Drawer.Root direction={isLaptop ? 'right' : 'bottom'}>
      ...
      <Drawer.Content
        ...
        style={
          isLaptop ? ({ '--initial-transform': 'calc(100% + 8px)' } as React.CSSProperties) : {}
        }
      >
        ...
        <Drawer.Title className="bg-background text-offgray-600 dark:text-offgray-200 inline-flex items-center justify-between px-4 pt-8 pb-2 text-lg font-bold">
          ...
          <span className="text-offgray-500 font-lora text-[12px] font-semibold italic">
            {t('menu.choosen')}: {countries.find((item) => item.href === locale)?.nativeName}
          </span>
        </Drawer.Title>
          ...
          {countries.map((item, index) => (
            <button
              ...
              className={cn(
                ...
                locale === item.href ? 'bg-offgray-100/50 dark:bg-offgray-500/10' : '',
              )}
              ...
              onClick={() => {
                ...
                changeLocale(item.href as Parameters<ReturnType<typeof useChangeLocale>>[0]);
              }}
            >
              <Image
